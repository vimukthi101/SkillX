...
jQuery(document).ready(function() {
	...
	/*jQuery("a:not('.myprofilelink, .jobalertlink')").click(function(e) {
	  e.preventDefault();
	});*/
	
	/*jQuery( ".btn:not('.btn-block, .btn-exd')" ).bind( "click", function() {
	 return false;
	});*/
	
	jQuery( ".btn-off" ).bind( "click", function() {
	 return false;
	});
	...
	jQuery('#sf-pricingtable-wrap').removeClass().addClass(function(){
	 ...
	 jQuery('.sf-radius-filter').slider({
	  formatter: function(value) {
	 ...
	 jQuery(".search-form .sf-radius-filter").on("slideStop", function(slideEvt) {
		...
		if(srhaddress == ""){
			alert(param.radius_search);	
			jQuery(this).slider('setValue', 0);
		}
	});
